...
class TestCheckArgumentJsonSchema:
    ...
    def test_string_pattern_valid(self):
        ...
        check_argument(
            "email", "test@example.com", {"type": "string", "pattern": r"^[\w]+@[\w]+\.\w+$"}
        )
    ...
    def test_string_pattern_invalid(self):
        ...
        with pytest.raises(PolicyValidationError):
            check_argument(
                "email", "not-an-email", {"type": "string", "pattern": r"^[\w]+@[\w]+\.\w+$"}
            )
    ...
    def test_array_valid(self):
        """Test valid array."""
        check_argument("items", ["a", "b", "c"], {"type": "array", "items": {"type": "string"}})

    def test_array_invalid_items(self):
        """Test array with invalid items."""
        with pytest.raises(PolicyValidationError):
            check_argument("items", ["a", 1, "c"], {"type": "array", "items": {"type": "string"}})
