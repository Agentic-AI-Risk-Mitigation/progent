...
class TestToolDefinition:
    ...
    def test_to_pydantic_model(self):
        ...
        tool = ToolDefinition(
            ...
            parameters=[
                ...
                ToolParameter(name="count", type="integer", description="Count", required=False, default=5),
            ],
            handler=lambda: None
        )
    ...
    def test_to_pydantic_model_missing_required(self):
        ...
        tool = ToolDefinition(
            ...
            description="A test tool",
            parameters=[
                ToolParameter(name="required_field", type="string", description="Required", required=True),
            ],
            handler=lambda: None
        )
    ...
    def test_all_tools_have_valid_parameters(self):
        ...
        for tool in TOOL_DEFINITIONS:
            for param in tool.parameters:
                assert param.type in valid_types, f"Invalid type '{param.type}' in tool '{tool.name}'"
                assert param.name
